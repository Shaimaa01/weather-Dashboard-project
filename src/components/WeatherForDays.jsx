...
const WeatherForDays = ({ city, isDarkMode }) => {
  ...
  useEffect(() => {
    ...
    const cachedData = JSON.parse(localStorage.getItem("weatherForDays"));
    if (cachedData) {
      try {
    ...
    const fetchForecast = async () => {
      try {
        ...
        localStorage.setItem("weatherForDays", JSON.stringify(data));
      } catch (err) {
   ...
   // Set an interval to fetch data at midnight
  ...
  return (
    <>
      ...
      {forecastData ? (
        <ul
          className={`${isDarkMode ? "custom-border" : "custom-border-light"}`}
        >
